...
def exportMapServerQueryShapefile(inQueryURL, inQueryParams, inOutputFormat, shpNullTypes, output_folder):
    ...
    try:
        ...
        if inQueryURL.find("query") == -1:
            # The word query is not in the URL
            log_message("**************************  The URL should include the word query!  **************************", "warn")
        ...
        if outputFormat == "SHAPEFILE":
            ...
            try:
                ...
                if debug: log_message("Delete Failed")
        ...
        if len(features) < query_cnt:
            log_message("************************************************", "warn")
            log_message("Map Service only returned " + str(len(features)) + " of " + str(query_cnt) + " features matching your query.", "warn")
            log_message("************************************************", "warn")

            fout.write("Map Service only returned " + str(len(features)) + " of " + str(query_cnt) + " features matching your query.")
        ...
        for feature in features:
            try:
                ...
                i += 1
                if i % 10 == 0:
                    if debug: log_message("Feature " + str(i) + ": " + strftime(time_fmt, gmtime()))
