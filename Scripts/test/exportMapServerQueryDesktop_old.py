...
# Make this very large for Desktop running
max_run_secs = 86400
    ...
    try:
        ...
        if export_all:
            ...
            while more_ids:
                ...
                if (time.time() - start_time) > max_run_secs:
            ...
            ''' 
            min_objectid = objectids[0]
            max_objectid = objectids[-1]
            x = range(min_objectid, max_objectid, max_features_returned)
            num_features_inserted = 0
            fout.write("**** Features rejected ****\n")
            for a in x:
                where = oid_field + ">=" + str(a) + " and " + oid_field + "<" + str(a + max_features_returned)
